...
Have a look at the slanggen.toml file; this contains the configuration for the data and model.
...
I published slanggen at [pypi](https://pypi.org/project/slangpy/) with uv (see `uv publish --help` for more info) which you can after building the wheel, making an account on [pypi.org](https://pypi.org/) and generating an API token from pypi to publish.
...
Finally:
- implement docker-compose.yml; this way you can make sure that the service starts up automatically after you pause and resume the instance on SURF.
